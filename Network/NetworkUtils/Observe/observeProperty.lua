...
const function observeProperty(instance: Instance, property: string, callback: (value: unknown) -> () -> ()): () -> ()
	local cleanFn: (() -> ())? = nil

	local propChangedConn: RBXScriptConnection
	local changedId = 0

	const function OnPropertyChanged()
		if cleanFn ~= nil then
			task.spawn(cleanFn)
			cleanFn = nil
		end
		...
		const value = (instance :: any)[property]
