...
const observePlayer = require(script.Parent.observePlayer)
...
const function observeCharacter(callback: (player: Player, character: Model) -> (() -> ())?): () -> ()
	return observePlayer(function(player)
		...
		const function OnCharacterAdded(character: Model)
			...
			-- Call the callback:
			task.defer(function()
				const cleanup = callback(player, character)
