...
const function defaultGuard(_value: AttributeValue)
	return true
end
...
const function observeAttribute(
	instance: Instance,
	name: string,
	...
	const valueGuard: (value: AttributeValue) -> boolean = if guard ~= nil then guard else defaultGuard

	const function OnAttributeChanged()
		if cleanFn ~= nil then
			task.spawn(cleanFn)
			cleanFn = nil
		end
