...
function observeTag<T>(tag: string, callback: (instance: T) -> (), ancestors: { Instance }?): () -> ()
	const instances: { [Instance]: InstanceStatus | () -> () } = {}
	const ancestryConn: { [Instance]: RBXScriptConnection } = {}

	local onInstAddedConn: RBXScriptConnection
	local onInstRemovedConn: RBXScriptConnection

	const function IsGoodAncestor(instance: Instance)
	...
	const function AttemptStartup(instance: Instance)
		...
		task.spawn(function()
			...
			const success, cleanup = xpcall(function(inst: T)
				const clean = callback(inst)
				if clean ~= nil then
			...
			if not success then
				local err = ""
				const firstLine = string.split(cleanup :: any, "\n")[1]
				const lastColon = string.find(firstLine, ": ")
				if lastColon then
	...
	const function AttemptCleanup(instance: Instance)
		const cleanup = instances[instance]
	...
	const function OnAncestryChanged(instance: Instance)
	...
	const function OnInstanceRemoved(instance: Instance)
		AttemptCleanup(instance)

		const ancestry = ancestryConn[instance]
		if ancestry then
			ancestry:Disconnect()
